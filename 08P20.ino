...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < dist_min || dist_raw > dist_max) {
    ...
    float absdist;
    int value;
    absdist = abs(dist_raw-200);
    value = 2.55*absdist; //2.55 is a constant that changes abs_distance into led_brightness_value.
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  return save; //return save which does not escape the distance.
